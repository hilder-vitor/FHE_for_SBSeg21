...
def test_cmux_gate(n, q, mult_depth = 5):
    N = ZZ(q/2)
    sigma = 3.2
    B = 4
    l = 10
    ...
    expec_msg = x^(27)
    c = gsw.encRLWE(expec_msg)


    for i in range(mult_depth):
        a_i = ZZ.random_element(-2*N, 2*N)
        s_i = ZZ.random_element(0, 2) # bit
        bk_i = gsw.enc(s_i)
        c = boot.cmux_gate(c, a_i, bk_i)
        expec_msg = RQ(expec_msg * x^(2*N - a_i * s_i))
    
        msg = gsw.decRLWE(c)

        assert(RQ(msg - expec_msg) == 0)

    print("OK")
    return True


def test_key_switching(n, q, num_tests = 10):
    N = 2^4
    sigma = 3.2
    B = 2
    l = ceil(log(q, 2))
    ...
    for i in range(num_tests):
        m = random_bit()
        s = poly_to_vec(gsw.sk, N)
        Delta = round(q / 4)
        a = [lwe_scheme.Zq.random_element() for _ in range(N)]
        a = vector(lwe_scheme.Zq, a) # converte lista em vetor

        e = lwe_scheme.random_noise()
        b = a*s + e + Delta * m

        c = [a, b]

        _c = boot.switch_key(c)

        _m = lwe_scheme.dec(_c, 1)
...
#test_cmux_gate(8, 2^5)
#test_key_switching(8, 2^20)
test_bootstrapp()
