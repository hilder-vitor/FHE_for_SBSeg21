...
#    def __str__(self):
#        return "LWE scheme: {n: %d, log q: %f, sigma: %f, q: %d}" \
#                 % (self.n, log(self.q,2), self.sigma, self.q)
    def __str__(self):
...
def test_lwe_scheme_enc_dec(lwe_scheme, num_tests=15):
    for i in range(num_tests):
        m = random_bit()
        c = lwe_scheme.enc(m)
        assert(lwe_scheme.dec(c, level=1) == m)
    return True

def test_lwe_scheme_nand(lwe_scheme, num_tests=25):
    for _ in range(num_tests):
        m0 = random_bit()
        m1 = random_bit()
        m_nand = 1 - m0*m1
        c0 = lwe_scheme.enc(m0)
        c1 = lwe_scheme.enc(m1)

        cnand = lwe_scheme.nand(c0, c1)
        dec_m = lwe_scheme.dec(cnand, level=2)
        assert(dec_m == m_nand)
...
def run_tests(lam = 50):
    ...
    if test_lwe_scheme_enc_dec(lwe_scheme, 25):
        print("test_lwe_scheme_enc_dec:       OK")

    if test_lwe_scheme_nand(lwe_scheme, 25):
        print("test_lwe_scheme_nand:  OK")
